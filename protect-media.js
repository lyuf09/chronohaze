(function () {
  ...
  function applySecondaryPageLanguage(lang) {
    ...
    if (document.body.classList.contains("search-index-page")) {
      ...
      var searchMetaDescription = safeLang === "en"
        ...
        : "Chronohaze 站内搜索：快速检索数学、摄影、音乐、研究与 CV 页面内容。";
  ...
  function setupFloatingSiteLogoContrast(wrapper) {
    ...
    function getDeclaredSurfaceLuminance(node) {
      if (!node || !node.closest) {
        return null;
      }
      var toneNode = node.closest("[data-floating-logo-tone]");
      if (!toneNode) {
        return null;
      }
      var tone = String(toneNode.getAttribute("data-floating-logo-tone") || "").toLowerCase();
      if (tone === "light") {
        return 1;
      }
      if (tone === "dark") {
    ...
    function estimateBackgroundImageLuminance(backgroundImage) {
      var value = String(backgroundImage || "");
      if (!value || value === "none") {
        return null;
      }
      var colorTokens = value.match(/rgba?\([^)]*\)/gi);
      if (!colorTokens || !colorTokens.length) {
        return null;
      }
      ...
      colorTokens.forEach(function (token) {
        var color = parseRgbColor(token);
        if (!color || color.a <= 0.02) {
          return;
        }
        weightedLuminance += getRelativeLuminance(color) * color.a;
        alphaSum += color.a;
        alphaAverage += color.a;
      });
      if (!alphaSum) {
        return null;
      }
      return {
        luminance: weightedLuminance / alphaSum,
        alpha: Math.max(0, Math.min(1, alphaAverage / colorTokens.length)),
      };
    }

    function getElementSurfaceLuminance(node, depth) {
      if (!node || depth >= 12) {
        return 0.5;
      }

      var declaredLuminance = getDeclaredSurfaceLuminance(node);
      if (declaredLuminance !== null) {
        return declaredLuminance;
      }
      ...
      var parentLuminance = getElementSurfaceLuminance(node.parentElement, depth + 1);
      if (!styles) {
        return parentLuminance;
      }

      var composedLuminance = parentLuminance;
      var hasSurfaceLayer = false;
      var bgColor = parseRgbColor(styles.backgroundColor);
      if (bgColor && bgColor.a > 0.02) {
        composedLuminance =
          getRelativeLuminance(bgColor) * bgColor.a + composedLuminance * (1 - bgColor.a);
        hasSurfaceLayer = true;
      }

      var imageEstimate = estimateBackgroundImageLuminance(styles.backgroundImage);
      if (imageEstimate) {
        composedLuminance =
          imageEstimate.luminance * imageEstimate.alpha +
          composedLuminance * (1 - imageEstimate.alpha);
        hasSurfaceLayer = true;
      }

      return hasSurfaceLayer ? composedLuminance : parentLuminance;
    }
    ...
    function getSurfaceLuminance(target, sampleX, sampleY) {
      ...
      return getElementSurfaceLuminance(node || document.body, 0);
    }
    ...
    function findSampleTarget(sampleX, sampleY) {
      ...
      if (typeof document.elementsFromPoint === "function") {
        stack = document.elementsFromPoint(safeX, safeY);
      } else {
        stack = [document.elementFromPoint(safeX, safeY)];
      }
      for (var i = 0; i < stack.length; i += 1) {
        if (stack[i] && !isInternalOverlayTarget(stack[i])) {
          return {
            target: stack[i],
            x: safeX,
            y: safeY,
    ...
    function sampleContrastTarget() {
      ...
      for (var i = 0; i < points.length; i += 1) {
        var point = points[i];
        var picked = findSampleTarget(point.x, point.y);

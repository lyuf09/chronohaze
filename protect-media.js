(function () {
  ...
  function dedupeNavLinks() {
    Array.from(document.querySelectorAll(".nav")).forEach(function (nav) {
      if (nav.dataset.canonicalPrimaryNav === "1") {
        return;
      }
  ...
  function renderSharePanelLanguage(root) {
    ...
    var shareLauncher = root.querySelector(".site-share-fab");
    if (shareLauncher) {
      shareLauncher.setAttribute(
        "aria-label",
        dict.shareButtonAria || "Share this page"
      );
    }
  ...
  function bridgeMobileShareWithFloatingLogo() {
    ...
    logo.setAttribute(
      "aria-label",
      (dict && dict.shareButtonAria) || "Share this page"
    );
  ...
  function ensureSiteSharePanel() {
    ...
    launcher.setAttribute("aria-label", "Share this page");
    launcher.innerHTML =
      '<span class="site-share-fab-icon" aria-hidden="true">↗︎</span><span class="site-share-fab-label" data-share-i18n="shareButton">Share</span>';
  ...
  function getSecondaryPageDictionary(lang) {
    ...
    return {
      zh: {
        ...
        searchFallbackMath: "浏览技术笔记",
        searchFallbackPhoto: "浏览摄影",
        searchFallbackMusic: "浏览音乐",
        searchFallbackCV: "浏览学术档案",
        searchFallbackExternal: "站外搜索（site:lyuf09.github.io/chronohaze）",
      ...
      en: {
        ...
        searchFormAria: "Site search form",
        ...
        searchScopeCV: "Academic Profile",
        ...
        searchFallbackCV: "Browse Academic Profile",
        searchFallbackExternal: "External search (site:lyuf09.github.io/chronohaze)",
        ...
        utilityResearch: "Research Statement",
        utilityProjects: "Selected Work",
        utilityNotes: "Technical Notes",
        ...
        shareButton: "Share",
        shareButtonAria: "Share this page",
  ...
  function applySupernovaIntroInEnglish(safeLang) {
    ...
    var paragraphs = [
      "“Supernova” was written on the eve of my seventeenth birthday, before Ipomoea alba’s emotional throughline emerged. It belongs to no narrative series; instead, it captures an internal collapse before that emotional universe had formed.",
      "The song unfolds in a fictional mental space. Its lyrics point to no one; they turn toward a projected self—an “other side” produced by an inner split, a presence that wants to draw close yet remains impossible to understand.",
      "The “clock tower” is not a real location but a symbol of time, existence, and isolation. Beneath it, the individual confronts their own smallness on a cosmic scale.",
      "Structurally, the track moves through mixed time signatures, a rhythmic language I strongly favored in my early writing. The changing tension between sections and the gradual emotional swell echo the image implied by the title: eruption → expansion → collapse.",
      "In the second half, one passage deliberately echoes the atmosphere of “Bering Sea” by Inchaos as a tribute. After the track was released, the band’s guitarist, Aiten, told me he liked it and asked for the multitracks so that he could attempt a remix. At the time, that became an unexpectedly precious memory.",
      "Overall, “Supernova” has a deep-blue texture. Its emotion points not to a specific event but to existential pressure that swells, expands, and dissolves. The apparently extreme lines are best read as a literary rendering of adolescent inner tension, not as a literal account of any real scene.",
      "Many of my later works are about “losing someone.”<br />“Supernova” records something earlier: before anyone else enters the story, a person must first stare into their own emptiness.",
    ];

    intro.innerHTML = paragraphs.join("<br /><br />");
  }

  function applySupernovaMetaInEnglish(safeLang) {
    if (
      safeLang !== "en" ||
      !document.body ||
      !document.body.classList.contains("music-detail-page")
    ) {
      return;
    }
    var detailPath = (window.location.pathname || "")
    ...
    var metas = Array.from(
      document.querySelectorAll(".music-detail-article .music-detail-meta")
    );
    if (metas[0]) {
      metas[0].textContent = "Creation period · Mar–May 2022";
    }
    if (metas[1]) {
      metas[1].innerHTML =
        "Lyrics · Composition · Arrangement — HazezZ<br />" +
        "Guitar — Franklimn Zhang<br />" +
        "Mixing — Rinya";
    }
  }
  ...
  function applyAffizierenIntroInEnglish(safeLang) {
    ...
    var paragraphs = [
      ...
      "The line “vision shattered by rain” returns to the summer when I was seventeen—a youthfully fragile illusion that only rain could completely destroy.",
  ...
  function applyAffizierenNotesInEnglish(safeLang) {
    ...
    var blocks = [
      ...
      "Opening: The track begins with a minimal, repeating piano figure. I deliberately used the piano sound from Guitar Pro instead of a velocity-shaped DAW instrument. The DAW pianos never produced the coldness I wanted; Guitar Pro gave the part the right mechanical, inorganic texture.",
      ...
      "Tuning: I used A♯–E–A–D–G–B–E (raised A), slightly higher than my usual drop A, to suit the key. The riff itself is not difficult; triplets soften the groove. The bass part became more interesting during recording: I first wrote it for hybrid picking, then switched to slap because it felt more natural.",
      "Verse meter play: The first verse uses only vocals (still Miku V4C) and clean guitar. A short metric cycle runs beneath it: bar 2 shifts from 4/4 to 9/8, bar 3 returns to 4/4, and bar 4 moves to 7/8 before the pattern repeats.",
      "Because the texture is sparse, those changes register as subtle instability rather than an overt rhythmic break, like a shadow swaying in the dark.",
      ...
      "Chorus design: The chorus begins with vocals and piano alone for eight bars, creating a sudden sense of emptiness. The full band then enters in one surge. Rhythm guitars remain chordal and restrained because atmosphere matters more here than technical display, while a slap variation of the main riff keeps the bass line continuous. One piano note deliberately sits just outside the expected color, creating a small offbeat tension.",
      ...
      "Solo writing: The instrumental solo is heavily influenced by DualInsomiNa. The rhythm guitars become almost non-repeating and line-driven, pushing forward through momentary tension while the picked bass follows underneath. The non-repeating guitar line makes this section deliberately difficult to reproduce live. The first two notes also deliberately echo a solo he once played on another song of mine.",
  ...
  var MUSIC_ROLE_PROFILES = {
    affizieren: {
      zh: ["作词", "作曲", "编曲", "吉他", "贝斯", "编程", "混音", "制作"],
      en: [
        "Lyrics",
        "Composition",
        "Arrangement",
  ...
  function applyAffizierenMeta(safeLang) {
    if (!document.body || !document.body.classList.contains("music-detail-page")) {
      return;
    }
    var detailPath = (window.location.pathname || "")
    ...
    if (metas[0]) {
      metas[0].textContent =
        safeLang === "en" ? "Creation period · Sep–Dec 2024" : "创作时间 · 2024 年 9–12 月";
    }
    if (metas[1]) {
      var roleLabels = MUSIC_ROLE_PROFILES.affizieren[safeLang === "en" ? "en" : "zh"];
      metas[1].innerHTML =
        (safeLang === "en" ? "Echoes of Two Years" : "两年的回响") +
        "<br /><br />" +
        roleLabels.join(" · ") +
        " — HazezZ";
    }
  ...
  function applyIpomoeaAlbumIntroInEnglish(safeLang) {
    ...
    var paragraphs = [
      {
        className: "album-authorship",
        copy: "Written, arranged, performed and produced by HazezZ, unless otherwise noted.",
      },
      {
        className: "album-note",
        copy:
      ...
      {
        className: "album-production-note",
        copy:
  ...
  function applyRainyDaysIntroInEnglish(safeLang) {
    ...
    var paragraphs = [
      ...
      "Musically, “Rainy days” threads in melodic motifs from Ipomoea alba again and again, like memory resurfacing in different corners. The song itself barely repeats sections. No turning back, no stopping—just moving forward all the way to the end, like time itself.",
    ];
  ...
  function renderSubmodularGreedyPost(safeLang, dict) {
    ...
    if (safeLang !== "en") {
      article.innerHTML = article.dataset.zhHtml;
      document.title = "早期项目记录：Submodular Greedy 的 Isabelle 形式化";
      return;
    }
    ...
    article.innerHTML = [
      ...
      "<h3><strong>Status at the Time of Writing</strong></h3>",
      "<p>In December 2025, this was still an ongoing research project. The components completed at that point included:</p>",
      "<ul>",
      ...
      "</ul>",
      "<h3><strong>Plans at the Time of Writing</strong></h3>",
      "<p>At the time, the next-stage plan was to:</p>",
      "<ul>",
    ...
    document.title =
      "Early Project Note: Formalising Submodular Greedy | Mathematics | Chronohaze";
  }
  ...
  function renderSpring2026Post(safeLang, dict) {
    ...
    article.innerHTML = [
      ...
      '<p class="article-meta">Feier Lyu · Jan 29, 2026</p>',
  ...
  function loadMusicCatalogMetadata() {
    ...
    musicCatalogLoadPromise = fetchJsonFromCandidates("assets/data/music-catalog.json")
      .then(function (payload) {
  ...
  function setupMusicIndexListeningRoom() {
    ...
    var selectedTracks = [
      {
        ...
        rolesZh: MUSIC_ROLE_PROFILES.affizieren.zh,
        rolesEn: MUSIC_ROLE_PROFILES.affizieren.en,
      ...
      section.appendChild(
        buildSectionHead(
          ...
          dict.musicFeaturedAlbumTitle ||
            textFor("专辑 / 概念项目", "Albums / Concept Projects"),
    ...
    function buildSelectedTracks(rowMetaMap, blueprints) {
      ...
      section.appendChild(
        buildSectionHead(
          textFor("聆听室", "LISTENING ROOM"),
          dict.musicSelectedTitle ||
            textFor("精选作品", "Listening Room / Selected Works"),
    ...
    function buildProductionNotes() {
      ...
      section.appendChild(
        buildSectionHead(
          textFor("制作", "PRODUCTION"),
          dict.musicProductionTitle || textFor("制作说明", "Production Notes"),
    ...
    function buildArchive(rowMetaMap) {
      ...
      section.appendChild(
        buildSectionHead(
          textFor("档案", "ARCHIVE"),
          dict.musicArchiveTitle || textFor("作品档案", "Archive"),
      ...
      if (albumMetas.length) {
        var albumGroup = buildArchiveGroup(
          dict.musicArchiveCollectionTitle || textFor("专辑与合集", "Collections")
        );
  ...
  function applyLocalizedPageMetadata(lang) {
    var safeLang = lang === "en" ? "en" : "zh";
    var titleNode = document.querySelector("title");
    var localizedTitle =
      titleNode && titleNode.getAttribute("data-title-" + safeLang);
    var descriptionNode = document.querySelector('meta[name="description"]');
    var localizedDescription =
      descriptionNode && descriptionNode.getAttribute("data-content-" + safeLang);
    if (localizedTitle) {
      document.title = localizedTitle;
    }
    if (localizedDescription) {
      descriptionNode.setAttribute("content", localizedDescription);
    }
  }

  function syncLanguageUrlAndSeo(lang) {
    var safeLang = lang === "en" ? "en" : "zh";
    ...
    var canonicalNode = document.querySelector('link[rel="canonical"]');
    var canonicalBase = new URL(canonicalNode.getAttribute("href"), window.location.href);
    canonicalBase.search = "";
    canonicalBase.hash = "";

    function variantHref(targetLang) {
      var variant = new URL(canonicalBase.toString());
      variant.searchParams.set("lang", targetLang);
    ...
    var canonicalHref = variantHref(safeLang);
    canonicalNode.setAttribute("href", canonicalHref);
    ensureAlternate("zh-CN", variantHref("zh"));
    ensureAlternate("en", variantHref("en"));
    ensureAlternate("x-default", canonicalBase.toString());

    setMetaTagContent('meta[property="og:url"]', canonicalHref);
    var descriptionNode = document.querySelector('meta[name="description"]');
    ...
    [
      ...
      if (entry[1]) setMetaTagContent(entry[0], entry[1]);
    });
  }
  ...
  function ensureSearchNavLink(lang) {
    ...
    Array.from(document.querySelectorAll(".site-header .nav")).forEach(function (nav) {
      ...
      nav.textContent = "";
      nav.dataset.canonicalPrimaryNav = "1";

      navOrder.forEach(function (key) {
        if (key === "studio") {
          var studioMenu = document.createElement("details");
          studioMenu.className = "nav-studio-menu";
          studioMenu.classList.toggle("active", currentKey === "studio");
          ...
          [
            ...
            studioLink.href = getPrimaryPageHref(entry[0]);
            studioLink.setAttribute("data-nav-key", entry[0]);
            studioLink.textContent = entry[1];
            var studioLinkIsActive =
              (entry[0] === "music" &&
                (isMusicIndexHref(window.location.pathname) ||
            ...
            studioPanel.appendChild(studioLink);
          });

          studioMenu.appendChild(studioTrigger);
          studioMenu.appendChild(studioPanel);
          nav.appendChild(studioMenu);
          return;
        }
  ...
  function ensureUnifiedPageLastUpdatedBadge(lang) {
    var safeLang =
      lang === "en" || lang === "zh" ? lang : detectPreferredLanguage();
    var dict = getSecondaryPageDictionary(safeLang);
    var dateIso = getCurrentPageLastUpdatedISO();

    if (
      document.body &&
      document.body.getAttribute("data-hide-last-updated") === "true"
    ) {
      Array.from(document.querySelectorAll(".page-last-updated-row")).forEach(
        function (row) {
          row.remove();
        }
      );
    ...
    if (!dateIso) {
      var incompleteRow = targetHost.querySelector(".page-last-updated-row");
      if (incompleteRow) {
  ...
  function applySecondaryPageLanguage(lang) {
    ...
    Array.from(document.querySelectorAll(".footer-copy")).forEach(function (node) {
      node.textContent = dict.footerCopy;
    });
    Array.from(document.querySelectorAll(".social")).forEach(function (node) {
      node.setAttribute("aria-label", dict.footerSocialAria);
    });
    ...
    Array.from(
      document.querySelectorAll(
        ".academic-local-nav a, a.academic-local-link, .academic-hub-links-label + .research-link-row a"
      )
    ).forEach(
      function (link) {
        var href = link.getAttribute("href") || "";
        if (isProjectsIndexHref(href)) {
          link.textContent = dict.utilityProjects;
        } else if (isResearchIndexHref(href)) {
          link.textContent = dict.utilityResearch;
        } else if (isMathIndexHref(href)) {
          link.textContent = dict.utilityNotes;
        }
      }
    );
    ...
    Array.from(document.querySelectorAll(".note-detail-back")).forEach(function (node) {
      node.textContent = dict.backToMath;
    });
    ...
    if (document.body.classList.contains("music-index-page")) {
      ...
      setMetaTagContent(
        ...
        safeLang === "en"
          ...
          : "HazezZ 的音乐作品页面：精选作品、专辑与概念项目、创作实践和完整作品档案。"
      );
      setMetaTagContent(
    ...
    if (document.body.classList.contains("math-index-page")) {
      ...
      document.title = safeLang === "en" ? "Technical Notes | Chronohaze" : "技术笔记 | Chronohaze";
    }

    if (document.body.classList.contains("search-index-page")) {
      var searchMetaTitle = safeLang === "en" ? "Search | Chronohaze" : "搜索 | Chronohaze";
      var searchMetaDescription = safeLang === "en"
        ? "Search Chronohaze across Technical Notes, photography, music, research, and Academic Profile content."
        : "Chronohaze 站内搜索：快速检索技术笔记、摄影、音乐、研究与学术档案内容。";
      document.title = searchMetaTitle;
      setMetaTagContent('meta[name="description"]', searchMetaDescription);
      setMetaTagContent('meta[property="og:title"]', searchMetaTitle);
      setMetaTagContent('meta[property="og:description"]', searchMetaDescription);
      setMetaTagContent('meta[name="twitter:title"]', searchMetaTitle);
      setMetaTagContent('meta[name="twitter:description"]', searchMetaDescription);
      var searchForm = document.querySelector(".search-form[role='search']");
      if (searchForm) {
        searchForm.setAttribute("aria-label", dict.searchFormAria);
      }
    }

    if (document.body.classList.contains("olfactory-page")) {
      var olfactoryTitle = safeLang === "en"
        ? "Olfactory Notes | Chronohaze"
        : "调香手记 | Chronohaze";
      var olfactoryDescription = safeLang === "en"
        ? "Invisible compositions made from tea, wood, flowers, resin, and memory."
        : "以茶、木、花、树脂与记忆为材料的气味作品与调香手记。";
      document.title = olfactoryTitle;
      setMetaTagContent('meta[name="description"]', olfactoryDescription);
      setMetaTagContent('meta[property="og:title"]', olfactoryTitle);
      setMetaTagContent('meta[property="og:description"]', olfactoryDescription);
      setMetaTagContent('meta[name="twitter:title"]', olfactoryTitle);
      setMetaTagContent('meta[name="twitter:description"]', olfactoryDescription);
    }
    ...
    if (document.body.classList.contains("music-detail-page")) {
      ...
      if (titleNode && titleNode.textContent) {
        ...
        var detailMetaTitle = titleText + " | Chronohaze";
        var detailMetaDescription = safeLang === "en"
          ? "Music work page for " + titleText + ", with background notes, audio playback, and lyrics or production details."
          : "音乐作品页面：" + titleText + "。包含作品介绍、音频播放器与歌词或制作信息。";
        setMetaTagContent('meta[property="og:title"]', detailMetaTitle);
        setMetaTagContent('meta[name="twitter:title"]', detailMetaTitle);
        setMetaTagContent('meta[name="description"]', detailMetaDescription);
        setMetaTagContent('meta[property="og:description"]', detailMetaDescription);
        setMetaTagContent('meta[name="twitter:description"]', detailMetaDescription);
      }
      ...
      applySupernovaMetaInEnglish(safeLang);
    ...
    if (
      ...
      if (safeLang === "en") {
        Array.from(
          document.querySelectorAll(".photo-detail-article .photo-detail-gallery img")
        ).forEach(function (image) {
          image.alt = localizeShortLabelText(image.alt || "", "en");
        });
      }
    ...
    applyLocalizedPageMetadata(safeLang);
    syncLanguageUrlAndSeo(safeLang);
  }

  function injectFloatingSiteLogo() {
    var existingFloatingLogo = document.querySelector(".floating-site-logo");
    if (existingFloatingLogo) {
      setupFloatingSiteLogoCollisionGuard(existingFloatingLogo);
    ...
    setupFloatingSiteLogoCollisionGuard(wrapper);
    setupFloatingSiteLogoContrast(wrapper);
    bridgeMobileShareWithFloatingLogo();
  }

  function setupFloatingSiteLogoCollisionGuard(wrapper) {
    if (!wrapper || wrapper.dataset.collisionGuardBound === "1") {
      return;
    }
    wrapper.dataset.collisionGuardBound = "1";

    var rafId = 0;
    var lastLift = -1;

    function isVisible(node) {
      if (!node || node.contains(wrapper)) {
        return false;
      }
    ...
    function syncLift() {
      ...
      if (window.innerHeight >= window.innerWidth) {
        Array.from(document.querySelectorAll("#playerShell, .persistent-audio-dock")).forEach(function (target) {
          if (!isVisible(target)) {
            return;
          }
          var targetRect = target.getBoundingClientRect();
          var overlapsHorizontally = logoRect.right > targetRect.left + 4 && logoRect.left < targetRect.right - 4;
          var overlapsNaturally = naturalBottom > targetRect.top && naturalTop < targetRect.bottom;
          if (overlapsHorizontally && overlapsNaturally) {
            neededLift = Math.max(neededLift, Math.ceil(naturalBottom - targetRect.top + 12));
          }
        });
      }

      neededLift = Math.max(0, Math.min(neededLift, Math.floor(window.innerHeight * 0.4)));
      if (neededLift === lastLift) {
        return;
      }
      lastLift = neededLift;
      wrapper.dataset.floatingLogoLift = String(neededLift);
      wrapper.style.setProperty("--floating-logo-lift", neededLift + "px");
      wrapper.classList.toggle("is-collision-lifted", neededLift > 0);
    }

    function requestSync() {
      if (!rafId) {
        rafId = window.requestAnimationFrame(syncLift);
      }
    }
    ...
    document.addEventListener("pause", requestSync, true);
    ...
    if (window.ResizeObserver) {
      var resizeObserver = new ResizeObserver(requestSync);
      [document.body, document.querySelector(".hero-split"), document.querySelector("#playerShell")].forEach(
        function (node) {
          if (node) {
    ...
    window.setTimeout(requestSync, 900);
    var settleChecks = 0;
    var settleInterval = window.setInterval(function () {
      requestSync();
      settleChecks += 1;
      if (settleChecks >= 10) {
  ...
  function syncPageStylesFromDocument(sourceDocument, targetUrl) {
    ...
    var managedStylesheetPattern =
      /(?:^|\/)(?:styles|home|performance-archive|editorial-evidence|academic-mobile|olfactory)(?:\.min)?\.css(?:[?#]|$)/;

    function isManagedStylesheetHref(href) {
      var input = String(href || "").toLowerCase();
      return managedStylesheetPattern.test(input);
    }
    ...
    var loadPromises = [];
    // Add destination-only styles after the current stylesheet group. Once
    // stale styles are removed below, this preserves the destination order
    // (base stylesheet first, then page-specific styles).
    var insertionAnchor = currentNodes.length
  ...
  window.ChronohazeShared.syncLanguageUrlAndSeo = syncLanguageUrlAndSeo;
